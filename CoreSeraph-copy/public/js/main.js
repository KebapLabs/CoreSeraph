class SocialLite {
  ...
  async handleGenerate(e) {
    ...
    // Check if moniker is a string and not empty after trimming
    if (!moniker || typeof moniker !== 'string' || moniker.trim().length === 0) {
      this.showMessage('Please enter a valid moniker', 'error');
      return;
    }
    try {
      const response = await fetch('/api/generate-key', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ moniker: moniker.trim() })
      });
      const data = await response.json();
