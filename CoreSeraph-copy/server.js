...
// Add child_process to execute yggdrasilctl
...
// Check for 'local' and 'ygg' command-line arguments
const isLocal = process.argv.includes('local');
const isYggdrasil = process.argv.includes('ygg');
const host = isYggdrasil ? null : (isLocal ? '0.0.0.0' : 'localhost'); // null for Yggdrasil mode (set later)

// -- YGGDRASIL AGGREGATOR -- //
/*
...
class YggdrasilClient {
    ...
    async register(siteInfo) {
        return new Promise((resolve, reject) => {
            if (!siteInfo.name || !siteInfo.address) {
                reject(new Error('Name and address required'));
                return;
            }
            ...
            client.on('close', () => {
                response.startsWith('SUCCESS') ? resolve(response) : reject(new Error(response));
            });
...
async function registerSite(siteInfo, host = 'localhost', port = 9001) {
    return new YggdrasilClient(host, port).register(siteInfo);
}

module.exports = { YggdrasilClient, registerSite };

if (require.main === module) {
    registerSite({
        name: 'Silica Social',
        address: '[203:9141:f6a0:4170:c5dc:70c2:33f4:3f43]:6767',
        description: 'A tiny forum meant to be a demo as a template for other forums on tor',
        tags: ['forum', 'demo', 'project', 'shits and giggles']
...
if (port < 1024) {
  console.error(`ERROR: Port ${port} requires root privileges. Use port >=1024`);
  process.exit(1);
}
...
// ... (Database schema creation remains unchanged) ...
db.serialize(() => {
...
// Function to get Yggdrasil IPv6 address
async function getYggdrasilAddress() {
  try {
    const { stdout } = await execAsync('yggdrasilctl getSelf');
    // Parse the plain text output to find the IPv6 address
    const ipv6Line = stdout.split('\n').find(line => line.startsWith('IPv6 address:'));
    if (!ipv6Line) return null;
    
    const address = ipv6Line.split(':').slice(1).join(':').trim();
    return address || null;
  } catch (err) {
    console.error('Error retrieving Yggdrasil address:', err.message);
    return null;
  }
}
...
// Start server with Yggdrasil support
(async () => {
  let serverHost = host;
  let serverAddress = host;

  if (isYggdrasil) {
    const yggAddress = await getYggdrasilAddress();
    if (!yggAddress) {
      console.error('Failed to retrieve Yggdrasil address. Is Yggdrasil running?');
      process.exit(1);
    }
    serverHost = yggAddress; // Bind to Yggdrasil IPv6 address
    serverAddress = `[${yggAddress}]`; // For display purposes (IPv6 addresses need brackets)
  }

  app.listen(port, serverHost, () => {
    console.log(`Silica Social running at http://${serverAddress}:${port}`);
    if (isYggdrasil) {
      console.log(`Yggdrasil mode enabled. Server bound to Yggdrasil IPv6 address: ${serverAddress}`);
    }
